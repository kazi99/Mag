...
\begin{example}
    ...
    \]
    are cohomological. Let us verify the first claim. Consider the long exact sequence 
    \[
    ...
    \]
    By proposition \ref{} we see that $\im f_* \subseteq \ker g_* $, so we are left to prove the other inclusion. Suppose $v \colon W \to Y$ is in $\ker g_*$. Then axiom TR3 (together with TR2 and TR1 (ii)) asserts the existence of a morphism $u \colon W \to X$ making the diagram below commutative.
    \[
        \begin{tikzcd}[column sep = normal, row sep = 2.5em]
            % https://q.uiver.app/#q=
            W & W & 0 & {W[1]} \\
...
The following are all specialized results characterizing, when a fully faithful triangulated functor is an equivalence. We will exploit corollary \ref{condition for functor to be an equivalence} in the proof of the final theorem of section \ref{}.
...
\begin{proof}
    See \cite[]{}. \info{make a reference}
\end{proof}
