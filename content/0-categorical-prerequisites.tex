...
In this section we mainly follow \cite[\S 8]{kashiwara2006categories}. With $k$ we denote either a field or the ring of integers $\Z$.
...
\begin{definition}  
    ...
    \]
    be a sequence of composable morphisms in an abelian category $\A$. We say this sequence is \emph{exact}, if $g \circ f = 0$ and the induced morphism $\im g \to \ker f$ is an isomorphism.
...
\begin{example}
    The entirety of Chapter \ref{Chapter: Derived categories} and first two sections of Chapter \ref{Chapter: Derived categories in geometry} will be devoted to constructing a substantial amount of triangulated categories and triangulated functors. The first triangulated category we will encounter in practice is the homotopy category of complexes $\kcat{\A}$ of an additive category $\A$ in Section \ref{Subsection: Categories of complexes} of this chapter.
\end{example}
...
\begin{proof}
    We will show that both $\id{\D}$ and $S_\D\inv \circ F\inv \circ S_{\D'} \circ F$ are left adjoint to the identity functor $\id{\D}$, making them naturally isomorphic by uniqueness of adjoints. For any two objects $X$, $Y$ of $\D$ we have a chain of natural isomorphisms in both $X$ and $Y$.
    \begin{align*}
