...
Additionally, we define the following full additive subcategories of $\com(\A)$.
\begin{center}
    \begin{tabular}{c l}
...
The homotopy category of complexes in $\A$, denoted by $\kcat{\A}$, is defined to be an additive category consisting of the following.
\begin{center}
    \begin{tabular}{r l}
        \textsl{Objects:} & chain complexes in $\A$. \\
...
A very important invariant of a chain complex in the homotopy category, which measures the extent to which it fails to be exact, is its cohomology. Here we are no longer assuming $\A$ is just $k$-linear, but abelian, since we will need kernels and cokernels to exist. 
