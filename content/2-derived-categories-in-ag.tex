...
By \cite[II, \S 9, Proposition 9.1.1]{EGA} the internal Hom $\localhom_{\struct{X}}(\F, \G)$ is coherent for any two coherent sheaves $\F$ and $\G$ on $X$. Therefore \eqref{eq: internal hom on O_X-modules} may be restricted to a bifunctor
\[
...
\begin{proposition}
    ...
    \begin{enumerate}[label = (\roman*)]
        \item{If $\G$ is a quasi-coherent shaef on $Y$, then $f^*\G$ is a quasi-coherent sheaf on $X$}
        \item{If $X$ and $Y$ are noetherain and $\G$ is a coherent sheaf on $Y$, then $f^*\G$ is a coherent sheaf on $X$.} 
    \end{enumerate}
\end{proposition}
