...
The proof of Proposition \ref{D+(I) = D+(A) F-adapted} hinges on the following lemma, closely resembling Lemma \ref{injective resolution}. In the same sprit, we call the complex $I^\bullet$ belonging to $\kplus{\I}$ together with a quasi-isomorphism $A^\bullet \to I^\bullet$ an \emph{$\I$-resolution} of the complex $A^\bullet$.
...
\]
denote the differential of $\Hom^\bullet(A^\bullet, C(u)^\bullet)$ and
\[
    ...
    \end{pmatrix}
\]
the differential of $C(u_*)^\bullet$. 
...
\begin{proof}
    ...
    \]
    therefore $\ker \delta^i_B$ is the set of all chain maps $A^\bullet \to B[i]^\bullet$, and $\im \delta^{i-1}_B$ the set of all null-homotopic chain maps.
\end{proof}
