...
\end{align*}
with these isomorphisms also commuting with the differentials. Pick $f \in \Hom^i(A^\bullet, C(u)^\bullet)$ which is given by a pair $f$
...
\begin{lemma}
    Let $A^\bullet$ and $B^\bullet$ be chain complexes in $\kcat{\A}$ and let $i \in \Z$. Then there are natural isomorphisms
    \[
        H^i()
    \]
\end{lemma}
