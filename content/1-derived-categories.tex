...
\begin{proposition}
    ...
    The constructed derived category $\dercat{\A}$ of an abelian category $\A$ together with the localization functor $Q \colon \kcat{\A} \to \dercat{\A}$ satisfies the universal property of definition \ref{universal property of D(A)}.
\end{proposition}
...
\begin{proof}
    ...
    % , where all the unmarked arrows are identities.
    \[\begin{tikzcd}[column sep = small, row sep = small]
    ...
    \end{gather*}
    The functor $F_0$ also commutes with the translation functors of $\dercat{\A}$ and $\D$, because $F$ is triangulated, and maps distinguished triangles of $\dercat{\A}$ to distinguished triangles of $\D$, making it a triangulated functor as well. Lastly it is clear from the definition, that $F_0 \circ Q = F$ and that $F_0$ is unique up to a natural isomorphism for which the diagram \eqref{universal property of D(A)} commutes. 
\end{proof}
...
\begin{proof}
    ...
    \info{this diagram doesn't need to take up so much space.}
...
\noindent
\textsc{Method 1.} Let $F\colon \A \to \B$ be a left exact functor between abelian categories with $\A$ having enough injectives. By Theorem \ref{D+ = K+ and injectives} we know that the composition of $\iota \colon \kplus{\J} \to \kplus{\A}$ followed by the localization functor $Q_\A \colon \kplus{\A} \to \derplus{\A}$ is an equivalence of triangulated categories. The \emph{right derived functor of $F$} is then defined to be the composition
\[
...
\begin{definition}
    \label{F-adapted}
    A class of objects $\I_F \subseteq \A$ is \emph{adapted} to a left\footnote{Dually, a class of objects $\mathcal P_F$ of $\A$ is \emph{adapted} to a \emph{right} exact functor $F \colon \A \to \B$ if it is closed under finite direct sums, every object of $\A$ is a quotient of some object of $\mathcal P$ and for every acyclic complex $P^\bullet$ in $\kminus{\A}$, with $P^i \in \mathcal P$, its image $F(P^\bullet)$ under $F$ is also acyclic.} exact functor $F \colon \A \to \B$, if the following three conditions are satisfied.
    \begin{enumerate}[label = (\roman*)]
...
The proof of Proposition \ref{D+(I) = D+(A) F-adapted} hinges on the following lemma, closely resembling Lemma \ref{injective resolution}. In the same sprit, we call the complex $I^\bullet$ belonging to $\kplus{\I}$ together with the quasi-isomorphism $A^\bullet \to I^\bullet$ an $\I$-resolution of the complex $A^\bullet$.
...
\] 
where $\derplus{\A} \xrightarrow{\ \iso \ } \derplus{\I}$ denotes a quasi-inverse to the equivalence $\derplus{\I} \iso \derplus{\A}$ established in Proposition \ref{D+(I) = D+(A) F-adapted}.
The procedure for computing the right derived functor $\rderived{}{F}(A^\bullet)$ of a complex $A^\bullet$ in $\derplus{\A}$ is then very similar to \textsc{Method 1}. One takes an $F$-adapted resolution $A^\bullet \to I^\bullet_A$ of $A^\bullet$ and then applies the functor $F$ to $I^\bullet_A$, \ie
\[
    \rderived{}{F}(A^\bullet) = F(I^\bullet_A).
\]

We mention that both methods give rise to right derived functors satisfying the following defining universal property. We only state this here, but refer the reader to \cite[\S III.6]{gelfand2002methods} for the claim and its proof or \cite[]{kashiwara2006categories} for a more comprehensive and high level treatment using the formalism of localization. 

\begin{definition}\cite[\S III.6, Definition 6]{gelfand2002methods}
    Let $F \colon \A \to \B$ be a left exact functor between abelian categories. The right derived functor of $F$ is a triangulated functor $\rderived{}{F} \colon \derplus{\A} \to \derplus{\B}$ together with a natural transformation $\varepsilon_F \colon Q_\B \circ F \Longrightarrow \rderived{}{F} \circ Q_\A$, such that for any triangulated functor $G \colon \derplus{\A} \to \derplus{\B}$ and any natural transformation $\varepsilon \colon Q_\B \circ F \Longrightarrow G \circ Q_\A$, there exists a unique natural transformation $\eta \colon \rderived{}{F} \Longrightarrow G$, for which
...
\begin{definition}
    For a right derived functor $\rderived{}{F} \colon \derplus{\A} \to \derplus{\B}$ of a left exact functor $F \colon \A \to \B$, we define the \emph{higher derived functors of} $F$ as
    \[
...
Frequently we will be interested in the image of a higher derived functor $\rderived{i}{F}$ of just a single object $A$ belonging to category $\A$. In that case we will always interpret\footnote{More precisely, there is always a fully faithful functor $\A \to \kplus{\A}$ sending an object $A$ to a complex concentrated in degree $0$ and likewise for morphisms \cite[Chapter 3, Lemma 1.3.5]{milicic-dercat}.} it as a bounded complex concentrated in degree $0$. 
...
\begin{definition}
    We say an object $A$ of $\A$ is \emph{$F$-acyclic}, if $\rderived{i}{F}(A) \iso 0$ for all $i \neq 0$.
\end{definition}
...
\begin{proof}
    ...
    \end{multline*}
    by Proposition \ref{LES in cohomology}, which is readily seen to be isomorphic to \eqref{eq: LES for left exact functor F}.  
\end{proof}
