...
\begin{lemma}
    \label{Ore condition}
    Let $s \colon A^\bullet \to B^\bullet$ be a quasi-isomorphism and let $f\colon C^\bullet \to B^\bullet$ be a morphism. Then there exist a quasi-isomorphism $u\colon C_0^\bullet \to C^\bullet$ and a morphism $g\colon C_0^\bullet \to A^\bullet$, such that the diagram below commutes in $K(\A)$.
    \[
    \begin{tikzcd}
        {C_0^\bullet} && C^\bullet \\
        \\
...
For fixed complexes $A^\bullet$ and $B^\bullet$ we define the hom-set\footnote{What will be defined here is a priori not even a set, so $D(\A)$, defined in this section, is not a category in the usual sense. We will however prove that in specific cases some variants of the derived category, especially concrete ones used later on, will from categories in the usual sense.} $\Hom_{D(\A)}(A^\bullet, B^\bullet)$ in the following way.
...
Note that since $C^\bullet \to C_0^\bullet \to A^\bullet$ is a quasi-isomorphism, the composite $C^\bullet \to C_1^\bullet \to A^\bullet$ is as well, concluding that $g\colon C^\bullet \to C^\bullet_1$ is a quasi-isomorphism. 
...
s \in \Hom_{K(A)}(C^\bullet, A^\bullet) \text{ quasi-isomorphism}
\end{array}
\right\}.
\]

Next we define the compositon operations
\[
...
\]
It can be shown that this is a well defined composition that is also associative. We leave out the proof, because it is routine. 
...
As with the homotopy category of complexes $K(\A)$, we also have the following bounded versions of the derived category $D(\A)$, exhibited as full additive subcategories of the unbounded variant $D(\A)$.
...
\subsubsection*{$k$-linear structure on $\mathrm{D}(\A)$}
We first define the biproduct on $D(\A)$.
\info{finish subsection}

\subsection{Derived categories of abelian categories with enough injectives}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= 0.
    \end{align*}
    The second equality follows from the inductive hypothesis and the third one from $f$ beign a chain map and $\delta$ being a differential. 

    Lastly, for $I^n$ is injecive and $j$ mono, there is a morphism $h^{n+1} \colon A^{n+1} \to I^n$, satisfying $h^{n+1} \circ j = g$, thus after precomposing both sides with $e$, we arrive at
    \[
        h^{n+1}d^n = f^n - \delta^{n-1}h^n.
        \qedhere  
    \]
\end{proof}
...
\begin{proof}[Proof of theorem \ref{D+ = K+ and injectives}]
    ...
    \]
    is a bijection, showing fully faithfulness. Essential surjectivity is clear by the existence of injective resolutions (\cf proposition \ref{injective resolution}).
\end{proof}
...
\subsection{Another source of subcategories of derived categories}