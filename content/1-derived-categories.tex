...
\begin{proof}
    ...
    \end{tabular}
    \end{center}
    This is well defined and functorial for the functor $F\colon \kcat{\A} \to \D$ sends quasi-isomorphisms of $\kcat{\A}$ to isomorphisms of $\D$. Moreover, $F_0$ is a $k$-linear functor, as can quickly be computed, applying lemma \ref{common denominator for left roofs} and its notation
    \begin{gather*}
        F_0(\phi_0 + \phi_1) = F(g_0 + g_1) \circ F(s)\inv = F(g_0) \circ F(s)\inv + F(g_1) \circ F(s)\inv = F_0(\phi_0) + F_1(\phi_1) \\
        F_0(\lambda \phi_0) = F(\lambda f_0) \circ F(s_0)\inv = \lambda (F(f_0) \circ F(s_0)\inv) = \lambda F_0(\phi_0).
    \end{gather*}
    The functor $F_0$ also commutes with the translation functors of $\dercat{\A}$ and $\D$, because $F$ is triangulated, and maps distinguished triangles of $\dercat{\A}$ to distinguished triangles of $\D$, making it a triangulated functor as well. Lastly it is clear from the definition, that $F_0 \circ Q = F$ and that $F_0$ is unique up to natural isomorphism making the diagram \eqref{universal property of D(A)} commute. 
\end{proof}
...
\begin{proof}
    ...
    \[\begin{tikzcd}[column sep = normal, row sep = 2.7em]
...
\begin{lemma}
    ...
    Let $F\colon \kcat{\A} \to \kcat{\B}$ be a triangulated functor, then $F$ preserves quasi-isomorphisms if and only if it preserves acyclic complexes.
    % \begin{enumerate}
...
\begin{enumerate}[label = $\triangleright$]
    ...
    \item Fix a left exact functor $F \colon \A \to \B$ and assume the category $\A$ contains a so-called \emph{$F$-adapted class}. Construct a \emph{right derived functor} $\rderived{}{F} \colon \derplus{\A} \to \derplus{\B}$.
\end{enumerate}
These methods are however no less restrictive than the case of exact functors, when viewed through the perspective of our applications later on. Lastly we note, that the first method is in some sense a special instance of the second one, which will be explained in this subsection.
