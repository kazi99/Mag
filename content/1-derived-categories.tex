...
There is also another (equivalent) way of obtaining the categories mentioned above, which will sometimes be more convenient to work with.  

\begin{proposition}
    \label{Subcategories of D(A) but bigger}
    Let $\A$ be an abelian category. 
    \begin{enumerate}[label = (\roman*)]
        \item{The inclusion $\derplus{\A} \hookrightarrow \dercat{\A}$ (resp.~$\derminus{\A} \hookrightarrow \dercat{\A}$) induces an equivalence between $\derplus{\A}$ and the full triangulated subcategory of $\dercat{\A}$ spanned on (possibly unbounded) complexes $A^\bullet$, for which there is an integer $n \in \Z$, such that $H^i(A^\bullet) \iso 0$ for all $i < n$ (resp.~$i > n$).}
        \item{}
        \item{} 
    \end{enumerate}
\end{proposition}

To make the proof of this proposition conceptually clearer, we introduce two complexes associated to a complex $A^\bullet$, namely
\begin{align*}
    \ltruncation{n}(A^\bullet) &= (\cdots \to 0 \to \coker d^{n-1} \to A^{n+1} \to A^{n+2} \to \cdots), \\
    \rtruncation{n}(A^\bullet) &= (\cdots \to A^{n-2} \to A^{n-1} \to \ker d^{n} \to 0 \to \cdots).
\end{align*}

They come equipped with the natural quotient map $q \colon A^\bullet \to \ltruncation{n}(A^\bullet)$ and the natural inclusion map $j \colon \rtruncation{n}(A^\bullet) \to A^\bullet$, satisfying the following
\begin{align}
    H^i(q) \colon H^i(A^\bullet) \to H^i(\ltruncation{n}(A^\bullet)) \text{ is an isomorphism for all } i \geq n, \\
    H^i(j) \colon H^i(\rtruncation{n}(A^\bullet)) \to H^i(A^\bullet) \text{ is an isomorphism for all } i \leq n.
\end{align}
...
\begin{proof}[Proof of Proposition \ref{Subcategories of D(A) but bigger}]
    The inclusion $\derplus{\A} \hookrightarrow \dercat{\A}$ is already fully faithful, so we verify only that it is essentially surjective onto the subcategory of complexes with cohomology bounded from below. Suppose $A^\bullet$ satisfies $H^i(A^\bullet) \iso 0$, for all $i < n$. Then we claim that the quotient map $q \colon A^\bullet \to \ltruncation{n}(A^\bullet)$ is actually a quasi-isomorphism.
...
\begin{proposition}
    The natural functor $\A \to \dercat{\A}$ is fully faithful, thus identifying $\A$ with the full subcategory of complexes $A^\bullet$ of $\dercat{\A}$, satisfying $H^i(A^\bullet) \iso 0$, for all $i \neq 0$.
\end{proposition}
...
\]
where $A$ is an object of an abelian category $\A$. Assume that category $\A$ contains enough injectives. By Example \ref{}, we know that $\Hom_\A(A, -)$ is a left exact functor, so we can right derive it, to obtain the functor
...
Next, we would like $\Hom^\bullet(A^\bullet, -)$ to descend to a homotopy functor $\kcat{\A} \to \kcat{\mod{k}}$. This is indeed so, as a null-homotopic chain map $u\colon B^\bullet \to C^\bullet$ is sent to a null-homotopic chain map $u_* \htpy 0$. Indeed, suppose homotopy $h \in \Hom^{-1}(B^\bullet, C^\bullet)$ whitnesses $u \htpy 0$, then for all $i \in \Z$, we have
\[
...
\] 
is well-defined. The preceding functor is also triangulated because it commutes with the translation functors and it sends distinguished triangles of $\kcat{\A}$ to distinguished triangles of $\kcat{\mod{k}}$ because of the following isomorphism of chain complexes of $k$-modules,
\[
...
\begin{proof}
    This is practically the definition of $\Hom_{\kcat{\A}}(A^\bullet, B[i]^\bullet)$. The cohomology $k$-module $H^i(\Hom^\bullet(A^\bullet, B^\bullet))$ is defined to be the quotient $\ker d^i / \im d^{i-1}$, thus let us identify $\ker d^i$ and $\im d^{i-1}$. Notice, that for $f \in \Hom^i(A^\bullet, B^\bullet)$ we have
    \[
        d^i(f) = \left(d^{i+j}_B \circ f^j + (-1)^{i+1} f^{j+1} \circ d^j_A\right)_{j \in \Z} = (-1)^i \left(d^j_{B[i]} \circ f^j - f^{j+1} \circ d^j_A\right)_{j \in \Z},
    \]
    thus $\ker d^i$ is the set of all chain maps $A^\bullet \to B[i]^\bullet$, and $\im d^{i-1}$ the set of all null-homotopic chain maps.
\end{proof}
...
\begin{proof}[Proof of Proposition \ref{Homs in derived category}]
    ...
    \begin{align*}
        ...
        &\iso \Hom_{\kplus{\J}}(A^\bullet, I[i]^\bullet) \qquad \qquad \text{(Theorem \ref{D+ = K+ and injectives})}\\
        &= H^i(\Hom^\bullet(A^\bullet, I^\bullet)) \qquad \qquad \quad \text{(Lemma \ref{Homotopy hom = cohomology of inner hom})} \\
