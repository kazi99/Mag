...
\begin{definition}
    ...
    \] 
    for all $i \in \Z$. If $f^\bullet$ and $g^\bullet$ are homotopic, we denote it by $f^\bullet \htpy g^\bullet$. 
    \[\begin{tikzcd}
...
\begin{lemma}
    ...
    \begin{enumerate}[(i)]
        \item The set of all nullhomotopic chain maps $A^\bullet \to B^\bullet$ forms a submodule of $\Hom(A^\bullet, B^\bullet)$.
...
\begin{center}
    ...
    \end{tabular}
\end{center}
\info{everywhere unless stated otherwise our complexes in $K^+(\A)$ will be supported in $\N$}
...
\begin{remark}
    ...
    \end{align*}
    We refre the reader to \cite[Definition 8.3.8.~(i)]{kashiwara2006categories}.
\end{remark}
...
The main goal of this section is to assign an appropriate triangulated functor on the level of derived categories, called a \emph{derived functor} of $F$ to a sensible additive functor $F \colon \A \to \B$ between two abelian categories. We will first deal with the easiest case, when $F$ is exact and then move on to the case, where $F$ will only be assumed to be left or right exact and instead require the domain category $\A$ to posses some nice properties (\eg contain enough injectives or contain an $F$-adapted class of objects). In practise the additional conditions $\A$ is required to satify are not very restricitve. 
...
\begin{proof}
    ...
    \]
    defined as follows. A morphism $\phi \colon A^\bullet \to B^\bullet$ represented by a roof $A^\bullet \xleftarrow{\sim} C^\bullet \to B^\bullet$ is sent to the equivalence class of the roof $F(A^\bullet) \xleftarrow{\sim} F(C^\bullet) \to F(B^\bullet)$, where it was crutial that $F$ preserves quasi-isomorphisms. By functoriality of $F$ it is evident, that this map is well defined. Clearly $D(\A) \to D(\B)$ fits into the commutative diagram \eqref{eq: exact functor derives}, is also $k$-linear by the $k$-linearity assumption on $F$, commutes with the respective translation functors and maps distinguished triangles to distingushed triangles, because of commutativity of diagram \eqref{eq: exact functor derives}, making it a triangulated functor.
\end{proof}
...
\begin{definition}
    \label{F-adapted}
    A class of objects $\I \subset \A$ is \emph{adapted} to a left exact functor $F \colon \A \to \B$, if the following three conditions are satisfied.
    \begin{enumerate}[(i)]
...
\begin{remark}
    We imediately observe, that whenever $\A$ contains enough injectives, the class of all injective objects forms an $F$-adapted class for \emph{every} left exact functor ${F\colon \A \to \B}$.
\end{remark}
...
\]
It is then clear that $K^+(F)$ is triangulated and it descends to a well defined functor on the level of derived categories 
\[
    D^+(\J) \to D^+(\B)
\]
...
\begin{proof}[Proof of proposition \ref{Derived cat of F-adapted category}]
    ...
    \end{equation}
    is injective for any objects $I^\bullet$, $J^\bullet$ of $D^+(\J)$. Suppose two morphisms $\phi_0$, $\phi_1 \colon I^\bullet \to J^\bullet$, repersented by right roofs $I \to I_0 \xleftarrow{\sim} J$ and $I \to I_1 \xleftarrow{\sim} J$, are sent to the same morphism in $\Hom_{D^+(\A)}(I^\bullet, J^\bullet)$, meaning that there is the following commutative diagram in $K^+(\A)$.
    \[\begin{tikzcd}[column sep = small, row sep = 1em]
        % https://q.uiver.app/#q=WzAsNSxbMSwyLCJJXzAiXSxbMywyLCJBIl0sWzAsNCwiSSJdLFs0LDQsIkoiXSxbMiwwLCJCIl0sWzIsMF0sWzMsMCwiXFxzaW0iXSxbMiwxXSxbMywxLCJcXHNpbSIsMl0sWzAsNF0sWzEsNCwiXFxzaW0iLDJdXQ==
        && A^\bullet \\
        \\
    ...
    \end{tikzcd}\]
    Then simply extending the diagram by a quasi-isomorphism $A \to I_2$, ensured by lemma \ref{F-acyclic resolutions}, proves that $\phi_0$ and $\phi_1$ were in fact equal and that the action is faithful.
...
\begin{proof}[Proof of proposition \ref{F-acyclic is F-adapted}]
    ...
    \info{There are inconsistencies with $F(A^\bullet) = K^+(F)(A^\bullet)$ and so on.} \qedhere
\end{proof}